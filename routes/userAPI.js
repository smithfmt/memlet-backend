...
router.get("/explore",
  passport.authenticate("jwt", {session: false}),
  async (req, res, next) => {
    let wordlists = await prisma.wordlist.findMany();
...
router.put("/upvote",
  ...
  async (req, res, next) => {
    ...
    let newUpvoted = !upvoted.includes(userId) ? [...upvoted, userId] : upvoted.filter(id => id!==userId);
    prisma.wordlist.update({
      where: {
        id,
      },
...
router.post("/create-folder",
  ...
  async (req, res, next) => {
    ...
    if (success) {
      res.status(200).json({ success: true, msg: "Successfully created Folder", createdFolder, success });
    } else {
